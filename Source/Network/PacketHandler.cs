...
,0x72 ,0x76 ,0x65 ,0x72                                         // ;rver
                                ...
                                case ItemUsage.Ping:
                                    {
                                        if (Time32.Now < client.LastPing.AddSeconds(2))
                                        {
                                            ...
                                            {
                                                Console.WriteLine(client.Entity.Name + " Nuevo hacker detectado con la ip {" + client.Account.IP + "}");
                                                Conquer_Online_Server.ServerBase.Kernel.SendWorldMessage(new Message("El Usuario " + client.Entity.Name + " ha utilizado hacker y se ha madando a la JailBot.", System.Drawing.Color.Blue, Network.GamePackets.Message.Center), Conquer_Online_Server.ServerBase.Kernel.GamePool.Values);
                                                client.Entity.Teleport(7852, 31, 74);
                                                client.Entity.PuntosJailBot += 1;
                                                ...
                                                zattack.Attacked = client.Entity.UID;
                                                zattack.AttackType = Network.GamePackets.Attack.Kill;
                                                zattack.X = client.Entity.X;
                                                zattack.Y = client.Entity.Y;
                                            }
                                        }
                                ...
                                case ItemUsage.RedeemGear:
                                    {
                                        ...
                                        if (item != null)
                                        {
                                            ...
                                            {
                                                client.Send(new Message("This item is expired!", System.Drawing.Color.White, Message.TopLeft));

                                                return;
                                            }
                                            if (client.Entity.ConquerPoints >= item.ConquerPointsCost)
                                            {
                                                client.Entity.ConquerPoints = (uint)Math.Max(0, (uint)((uint)client.Entity.ConquerPoints - (uint)item.ConquerPointsCost));
                                                ...
                                                if (ServerBase.Kernel.GamePool.ContainsKey(item.GainerUID))
                                                {
                                                    ...
                                                    if (pClient.ClaimableItem.ContainsKey(item.UID))
                                                    {
                                                ...
                                                Message message = new Message(item.OwnerName + " has redeemed the detained items from the arrest and " + item.GainerName + " has received the ransom as a reward.", System.Drawing.Color.Wheat, Message.TopLeft);
                                                ServerBase.Kernel.SendWorldMessage(message, ServerBase.Kernel.GamePool.Values);
                                            }
                                        }
                                        else
                                        {
                                            client.Send(new Message("The item you want to redeem has already been redeemed.", System.Drawing.Color.White, Message.TopLeft));
                                        }
                                        break;
                                    }
                                case ItemUsage.ClaimGear:
                                    {
                                        ...
                                        if (item != null)
                                        {
                                            if (item.Bound)
                                            {
                                                client.Send(new Message("Unnclaimable item!", System.Drawing.Color.White, Message.TopLeft));
                                                return;
                                            }
                                            if (DateTime.Now < item.Date.AddDays(7) && item.OwnerUID != 500)
                                            {
                                                client.Send(new Message("This item is not expired. You cannot claim it yet!", System.Drawing.Color.White, Message.TopLeft));
                                                return;
                                            }
                                        ...
                                        {
                                            client.Send(new Message("The item you want to claim has already been claimed.", System.Drawing.Color.White, Message.TopLeft));
                                        }
                                        break;
                                    }
                                case 34:
                ...
                case 2032:
                    {
                        ...
                        if (client.Map.BaseID != 6001 && !client.Entity.Dead && req.OptionID == 249)
                            if (client.Map.BaseID != 6001 && !client.Entity.Dead && req.OptionID == 230)
                            {
                                ...
                                {
                                    if (client.Entity.NobilityRank == Game.ConquerStructures.NobilityRank.King && Program.nobilityrankwar == true)
                                    {
                                        #region
                                    ...
                                    else if (client.Entity.NobilityRank == Game.ConquerStructures.NobilityRank.Prince && Program.nobilityrankwar == true)
                                    {
                                        #region
                                    ...
                                    else if (client.Entity.NobilityRank == Game.ConquerStructures.NobilityRank.Duke && Program.nobilityrankwar == true)
                                    {
                                        #region
                                    ...
                                    {
                                        client.Send(new Network.GamePackets.NpcReply(6, "Sorry The War Avilable For  (King-Princes-Duke) Only."));
                                    }
                                }
                            }
                        {
                ...
                case 1040:
                    {
                        Conquer_Online_Server.Network.GameBufferets.Status ShowStatistics2 = new Conquer_Online_Server.Network.GameBufferets.Status(client);
                ...
                default:
                    {
                        ...
                        if (ID == 1040)
                            client.Send(packet);
                        if (ID==2064)
                            client.Send(new Message("Packet de Novility Bien", System.Drawing.Color.CadetBlue, Message.Talk));
                        break;
                    }
            }
        }
     ...
     (ushort)(client.ExtraAtributePoints(client.Entity.FirstRebornClass, client.Entity.FirstRebornLevel) +
      client.ExtraAtributePoints(client.Entity.SecondRebornClass, client.Entity.SecondRebornLevel) + 62);
                ...
                switch (client.Entity.FirstRebornClass)
                {
                    ...
                    case 55:
                        {
                            Add(Conquer_Online_Server.Game.Enums.SkillIDs.CannonBarrage);
                            ...
                            case Conquer_Online_Server.Database.ShopFile.MoneyType.ConquerPoints:
                                {
                                    ...
                                    {
                                        if (iteminfo.BaseInformation.ConquerPointsWorth > client.Entity.BoundCps)
                                            return;
                                        if (client.Entity.BoundCps - iteminfo.BaseInformation.ConquerPointsWorth > client.Entity.BoundCps)
                                            return;
                                        ...
                                        client.Inventory.Add(item, Game.Enums.ItemUse.CreateAndAdd);
                                        if (client.Inventory.ContainsUID(item.UID) || item.StackSize == 1)
                                            client.Entity.BoundCps = (uint)Math.Max(0, (int)((int)client.Entity.BoundCps - (int)iteminfo.BaseInformation.ConquerPointsWorth));
                                    }
                                    else
                                    {
                                        ...
                                        client.Inventory.Add(item, Game.Enums.ItemUse.CreateAndAdd);
                                        if (client.Inventory.ContainsUID(item.UID) || item.StackSize == 1)
                                            client.Entity.ConquerPoints = (uint)Math.Max(0, (int)((int)client.Entity.ConquerPoints - (int)iteminfo.BaseInformation.ConquerPointsWorth));
                                    }
                                    break;
                                }
                        }
        ...
        public static ushort ItemPosition(uint ID)
        {
            ...
            if (ID == 134155 || ID == 131155 || ID == 133155 || ID == 200413)
        ...
        static bool EquipPassJobReq(Database.ConquerItemBaseInformation baseInformation, Client.GameState client)
        {
            ...
            {
                #region PM
                case 30: if (client.Entity.Class <= 35 && client.Entity.Class >= 30) return true; break;
                case 31: if (client.Entity.Class <= 35 && client.Entity.Class >= 31) return true; break;
                case 32: if (client.Entity.Class <= 35 && client.Entity.Class >= 32) return true; break;
                case 33: if (client.Entity.Class <= 35 && client.Entity.Class >= 33) return true; break;
                case 34: if (client.Entity.Class <= 35 && client.Entity.Class >= 34) return true; break;
                case 35: if (client.Entity.Class == 35) return true; break;
               #endregion
                #region Pirate
                case 70: if (client.Entity.Class <= 75 && client.Entity.Class >= 0) return true; break;
...
,0x00 ,0x00 ,0x0B ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x54 ,0x51 ,0x53 ,0x65 ,0x72 ,0x76 ,0x65 ,0x72      //;       TQServer
                            ...
                            case "remove":
                                {
                                    if (client.Entity.Class >= 10 && client.Entity.Class <= 75)
                                    {
                                        if (!client.Equipment.Free(5))
                                        {
                                            client.Equipment.Remove(5);
                                            NpcReply x = new NpcReply(6, " Your Left Weapon is Successfuly UnEquipped ");
                                            x.OptionID = 87;
                                            client.Send(x);
                                        }
                                        else
                                        {
                                            NpcReply x1 = new NpcReply(6, " Your Dont have Left Weapon Equipped yet To Be UnEquipped ");
                                            x1.OptionID = 87;
                                            client.Send(x1);
                                        }
                                        break;
                                    ...
                                    {
                                        NpcReply x2 = new NpcReply(6, "You Are Taoist So Cant Equip LeftHand to be Able to UnEquip It");
                                        x2.OptionID = 87;
                                        client.Send(x2);
                                    }
                                }
        ...
        static void DoLogin(object _client)
        {
            ...
            #region Nuevos Jugadores by Fito
            if (client.Entity.Class == 0)
            {
                client.Entity.Class = 70;
            }
            if (client.Entity.Class >= 10 && client.Entity.Class <= 11 && client.Entity.Level == 1)
            {
                client.Inventory.Add(132005, 1, 1);
                client.Inventory.Add(410005, 1, 1);
                client.Entity.ConquerPoints = 27;
            }
            if (client.Entity.Class >= 20 && client.Entity.Class <= 21 && client.Entity.Level == 1)
            {
                client.Inventory.Add(132005, 1, 1);
                client.Inventory.Add(410005, 1, 1);
                client.Entity.ConquerPoints = 27;
            }
            if (client.Entity.Class >= 40 && client.Entity.Class <= 41 && client.Entity.Level == 1)
            {
                client.Inventory.Add(132005, 1, 1);
                client.Inventory.Add(500005, 1, 1);
                client.Entity.ConquerPoints = 27;
            }
            if (client.Entity.Class >= 50 && client.Entity.Class <= 51 && client.Entity.Level == 1)
            {
                client.Inventory.Add(132005, 1, 1);
                client.Inventory.Add(601005, 1, 1);
                client.Entity.ConquerPoints = 27;
            }
            if (client.Entity.Class >= 60 && client.Entity.Class <= 61 && client.Entity.Level == 1)
            {
                client.Inventory.Add(132005, 1, 1);
                client.Inventory.Add(610005, 1, 1);
                client.Entity.ConquerPoints = 27;
            }
            if (client.Entity.Class >= 70 && client.Entity.Class <= 71 && client.Entity.Level == 1)
            {
                client.Inventory.Add(132005, 1, 1);
                client.Inventory.Add(611005, 1, 1);
                client.Inventory.Add(612005, 1, 1);
                client.Entity.ConquerPoints = 27;
            }
            if (client.Entity.Class >= 100 && client.Entity.Class <= 101 && client.Entity.Level == 1)
            {
                client.Inventory.Add(132005, 1, 1);
                client.Inventory.Add(421005, 1, 1);
                client.Entity.ConquerPoints = 27;
            }
            #endregion
        ...
        static void LoginMessages(Client.GameState client)
        {
            ...
            client.Send(new GameUpdates(GameUpdates.Header, "MR.sayed-Co Updates" + DateTime.Now.ToString()));
            ...
            client.Send(new GameUpdates(GameUpdates.Body, "10.Only PM number 01278771180 for donate"));
            client.Send(new GameUpdates(GameUpdates.Footer, "sayed.elnams@yahoo.com[PM] has [PM] of  endes on his name dont trust any one else"));
            #endregion
